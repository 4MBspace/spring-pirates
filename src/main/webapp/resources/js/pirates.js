var incrementKills = function() {
	
    $.get("api/ikills", function(data, status){
        console.log("Kills++, " + "Data: " + data + ", Status: " + status);
    });
}

var incrementDeaths = function() {
	
    $.get("api/ideaths", function(data, status){
        console.log("Deaths++, " + "Data: " + data + ", Status: " + status);
    });
}
...
function preload () {
    ...
    game.load.image('buttonLeft', 'resources/assets/mobilebuttonleft.png');
    game.load.image('buttonRight', 'resources/assets/mobilebuttonright.png');
    game.load.image('buttonSlow', 'resources/assets/mobilebuttonslow.png');
    game.load.image('buttonFast', 'resources/assets/mobilebuttonfast.png');
    game.load.image('buttonFire', 'resources/assets/mobilebuttoncannonball.png');    
}
...
function create () {
    ...
    if(window.mobile)
	{
    	window.leftButton = game.add.button(window.gameWidth/2-50, 50, 'buttonLeft', rudderLeft, 'buttonLeft', 'buttonLeft', 'buttonLeft', 'buttonLeft');
    	window.rightButton = game.add.button(window.gameWidth/2-50, window.gameHeight-150, 'buttonRight', rudderRight, 'buttonRight', 'buttonRight', 'buttonRight', 'buttonRight');
    	window.fireLeftButton = game.add.button(100, 50, 'buttonFire', fireL, 'buttonFire', 'buttonFire', 'buttonFire', 'buttonFire');
    	window.fireRightButton = game.add.button(100, window.gameHeight-150, 'buttonFire', fireR, 'buttonFire', 'buttonFire', 'buttonFire', 'buttonFire');
    	window.fastButton = game.add.button(window.gameWidth-200, window.gameHeight-150, 'buttonFast', faster, 'buttonFast', 'buttonFast', 'buttonFast', 'buttonFast');
    	window.slowButton = game.add.button(window.gameWidth-200, 50, 'buttonSlow', slower, 'buttonSlow', 'buttonSlow', 'buttonSlow', 'buttonSlow');
    	  
    	window.leftButton.fixedToCamera = true;
    	window.rightButton.fixedToCamera = true;
    	window.fireLeftButton.fixedToCamera = true;
    	window.fireRightButton.fixedToCamera = true;
    	window.fastButton.fixedToCamera = true;
    	window.slowButton.fixedToCamera = true;
    	
    	//window.leftButton.anchor.setTo(0.5, 0.5);
    	//window.rightButton.anchor.setTo(0.5, 0.5);
    	//window.fireLeftButton.anchor.setTo(0.5, 0.5);
    	//window.fireRightButton.anchor.setTo(0.5, 0.5);
    	//window.fastButton.anchor.setTo(0.5, 0.5);
    	//window.slowButton.anchor.setTo(0.5, 0.5);
    	
    	//window.leftButton.angle = 90;
    	//window.rightButton.angle = 90;
    	//window.fireLeftButton.angle = 90;
    	//window.fireRightButton.angle = 90;
    	//window.fastButton.angle = 90;
    	//window.slowButton.angle = 90;
    	
    	window.game.world.bringToTop(leftButton);
    	window.game.world.bringToTop(rightButton);
    	window.game.world.bringToTop(fireLeftButton);
    	window.game.world.bringToTop(fireRightButton);
    	window.game.world.bringToTop(fastButton);
    	window.game.world.bringToTop(slowButton);
	}
}
//End Create
...
function rudderLeft()
{
	if(window.rudder > -window.maxRudder)
        window.rudder -= window.maxRudder/8;
	
	window.isStateChanged = true;
}
function rudderRight()
{
	if(window.rudder < window.maxRudder)
        window.rudder += window.maxRudder/8;
...
function update () {
    ...
    for (var [id, enemyState] of window.enemies){
        if(!idSet.has(id))
        {
            window.enemiesTotal--;
            window.enemiesAlive--;
            var enem = window.enemies.get(id);            
            var explosionAnimation3 = window.explosions.getFirstExists(false);
            explosionAnimation3.reset(enem.x, enem.y);
            enem.damage(100);
            explosionAnimation3.play('kaboom', 30, false, true);
